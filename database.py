...
def init_db():
    ...
    with get_db() as conn:
        ...
        # Must include both my_sig_info (activate two-fers) and dx_sig_info (hunt two-fers)
        if 'qsos' in tables:
            old_index = conn.execute(
                "SELECT sql FROM sqlite_master WHERE type='index' AND name='idx_qsos_unique'"
            ).fetchone()
            if old_index and 'dx_sig_info' not in (old_index[0] or ''):
                ...
                conn.execute("""
                    ...
                    ON qsos(competitor_callsign, dx_callsign, qso_datetime_utc,
                            COALESCE(my_sig_info, ''), COALESCE(dx_sig_info, ''))
                """)
                conn.commit()
        ...
        conn.executescript("""
            ...
            -- Unique constraint to prevent duplicate QSOs (includes sig_info fields to support two-fers)
            CREATE UNIQUE INDEX IF NOT EXISTS idx_qsos_unique
            ON qsos(competitor_callsign, dx_callsign, qso_datetime_utc, COALESCE(my_sig_info, ''), COALESCE(dx_sig_info, ''));
